...
class ResolverCliTests(unittest.TestCase):
    ...
    def test_in_file_attaches_locked_version_from_compiled_txt(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            in_file = Path(tmp) / "requirements.in"
            in_file.write_text(
                "requests~=2.31.0\nfastapi>=0.110.0\n", encoding="utf-8",
            )
            ...
            deps = cli.parse_requirements(in_file)
            by_name = {d.name.lower(): d for d in deps}
            self.assertEqual(by_name["requests"].locked_version, "2.31.4")
            self.assertEqual(by_name["fastapi"].locked_version, "0.128.8")

    def test_in_file_without_compiled_txt_has_none_locked_version(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            in_file = Path(tmp) / "requirements.in"
            in_file.write_text("requests~=2.31.0\n", encoding="utf-8")
            deps = cli.parse_requirements(in_file)
            self.assertIsNone(deps[0].locked_version)

    def test_txt_manifest_does_not_consult_sidecar(self) -> None:
        # A plain .txt manifest is itself the lockfile; no sidecar lookup.
        with tempfile.TemporaryDirectory() as tmp:
            manifest = Path(tmp) / "requirements.txt"
